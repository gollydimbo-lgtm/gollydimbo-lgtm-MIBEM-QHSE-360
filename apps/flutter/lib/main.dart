...
@override Widget build(BuildContext c)=>Scaffold(appBar:AppBar(title:const Text('Nouveau contrôle')),body:ListView(padding:const EdgeInsets.all(16),children:[TextField(controller:code,decoration:const InputDecoration(labelText:'Code contrôle')),TextField(controller:lot,decoration:const InputDecoration(labelText:'Numéro de lot')),const SizedBox(height:10),_drop('Site',siteId,sites.map((x)=>DropdownMenuItem(value:x['id'],child:Text(x['name']))).toList(),(v){siteId=v;final s=sites.firstWhere((x)=>x['id']==v);setState((){lines=List.from(s['lines']??[]);lineId=null;machineId=null;});}),_drop('Ligne',lineId,lines.map((x)=>DropdownMenuItem(value:x['id'],child:Text(x['name']))).toList(),(v){lineId=v;final l=lines.firstWhere((x)=>x['id']==v);setState(()=>machines=List.from(l['machines']??[]));}),_drop('Machine',machineId,machines.map((x)=>DropdownMenuItem(value:x['id'],child:Text(x['name']))).toList(),(v)=>setState(()=>machineId=v)),_drop('Produit',productId,products.map((x)=>DropdownMenuItem(value:x['id'],child:Text(x['name']))).toList(),(v){productId=v;final p=products.firstWhere((x)=>x['id']==v);setState(()=>formats=List.from(p['formats']??[]));}),_drop('Format',formatId,formats.map((x)=>DropdownMenuItem(value:x['id'],child:Text(x['label']))).toList(),(v)=>setState(()=>formatId=v)),_drop('Quart',shiftId,shifts.map((x)=>DropdownMenuItem(value:x['id'],child:Text(x['name']))).toList(),(v)=>setState(()=>shiftId=v)),_drop('Modèle de contrôle',templateId,templates.map((x)=>DropdownMenuItem(value:x['id'],child:Text(x['name']))).toList(),(v)=>setState(()=>templateId=v)),const SizedBox(height:12),OutlinedButton.icon(onPressed:gps,icon:const Icon(Icons.gps_fixed),label:Text(lat==null?'Capturer GPS':'GPS ${lat!.toStringAsFixed(5)}, ${lon!.toStringAsFixed(5)}')),const SizedBox(height:18),FilledButton.icon(onPressed:busy?null:create,icon:const Icon(Icons.play_arrow),label:Text(busy?'Création...':'Démarrer le contrôle'))]));}
